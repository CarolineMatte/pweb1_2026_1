...
if (isset($_GET['excluir_id'])) {
    ...
    try {
        if ($stmt->execute()) {
            $mensagem_exclusao = "<div class='alert alert-success'>Cliente excluído com sucesso!</div>";
        } else {
            $mensagem_exclusao = "<div class='alert alert-danger'>Erro ao excluir cliente.</div>";
        }
    } catch (PDOException $e) {
        if ($e->getCode() == 23000) {
            $mensagem_exclusao = "<div class='alert alert-warning'>Exclusão bloqueada: existem reservas vinculadas a este cliente. Apague as reservas primeiro.</div>";
        } else {
